...
export default function LoginPage() {
  ...
  // Auto-focus TOTP input when step appears
  useEffect(() => {
    if (totpRequired) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Admin with 2FA enabled → show TOTP step
  ...
  const onTotpSubmit = async (e) => {
    ...
    setTotpLoading(true);
    try {
      const res = await api.post('/auth/totp/verify-login', { tempToken, code: totpCode });
  ...
  const handleGoogleLogin = () => {
    ...
    window.location.href = `${baseURL}/auth/google?origin=${origin}`;
  };
          ...
          <AnimatePresence mode="wait">
            ...
            {totpRequired ? (
              ...
              >
                ...
                <form onSubmit={onTotpSubmit} className="space-y-5">
                  <div className="flex justify-center">
                    <input
                      ...
                      className="w-44 rounded-xl border border-neutral-300 bg-white px-4 py-3 text-center font-mono text-3xl tracking-[0.4em] focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-200"
                    />
                  </div>
                ...
                <button
                  type="button"
                  onClick={() => { setTotpRequired(false); setTotpCode(''); setTempToken(''); }}
                  className="mt-5 w-full text-center text-sm text-neutral-400 hover:text-neutral-600"
                >
                  ← Back to login
                </button>
              </motion.div>
